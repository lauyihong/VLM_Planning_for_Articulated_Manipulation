...
def main():
    ...
    cabinet_scale = float(os.environ.get("CABINET_SCALE", "0.6"))
    cabinet_z = float(os.environ.get("CABINET_Z", "0.5"))
    print(f"[generic] URDF={cabinet_urdf} scale={cabinet_scale} z={cabinet_z}", flush=True)

    loader.scale = cabinet_scale
    cabinet = loader.load(cabinet_urdf)
    cabinet.set_pose(sapien.Pose(p=[0.8, 0.0, cabinet_z]))
    loader.scale = 1.0
    ...
    _cam_target_z = float(os.environ.get("CAM_TARGET_Z", "0.6"))
    _cam_eye_z    = float(os.environ.get("CAM_EYE_Z", "1.0"))
    cam_eye, cam_target = np.array([-0.4, -1.0, _cam_eye_z]), np.array([0.8, 0.0, _cam_target_z])
